 ...
 * to respond to the user with suitable message. if the data is convenient and there is username or email and password
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST) && !empty($json)) {
        ...
        if (isset($user->username)) {
            ...
            if ($userob->login($data["username"], $data["password"])) {
            ...
            if ($userob->login($data["email"], $data["password"])){
                $userInfo= $userob->getUserInfo ();
                echo json_encode(["token"=> $userInfo['token']]);
                exit();
            } else {
