...
char Open[40] = "openGarageDoor";
char Close[40] = "closeGarageDoor";
char ACK[40] = "ACK";
char NACK[40]= "NACK";
...
int loop(int garageState, int retry){
	radio.stopListening();
	...
	logFile<<"server sent garage State: "<<garageState<<endl;
	
	//write to arduino
	if(garageState == 0){ //0 measns close the door
		printf("sending mode: %s\n",Close);
		logFile<<"sending mode: "<<Close<<endl;
		ret = radio.write(&Close,sizeof(char[40]));
		radio.startListening();
	}
	else if(garageState == 1){ //1 means open the garage
		printf("sending mode: %s\n",Open);
	...
	while(!radio.available()){
		if(millis()-current_time > 15000){ //wait for 15 seconds
			timeout = true;
			break;
		}
	}
	...
	if(timeout){
		...
		if(num1 != 0){
			...
			else if(strcmp(incoming, NACK) == 0){
