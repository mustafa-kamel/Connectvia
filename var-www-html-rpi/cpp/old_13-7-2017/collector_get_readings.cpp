...
int readSLV(int retry){
	...
	if(!timeout){
		...
		if(!waterLeakageState){ //waterLeakage sensor is active low
			send_to_server = 1;
		}
...
int readSLV2(int retry){
	...
	if(!timeout){
		...
		if(PIRState2){
			send_to_server = 1; //introder detected, you should update database to inform admin
		}
		printf("PIR State = %d\n",PIRState2);
...
void sendToServer(){ //send all reading to server
	...
	command<<"0";						//whether the home door is open  or not
	//command<<"\"";
	system(command.str().c_str());
	...
	command.str("");
	//command<<"\"";
	command<<"php update.php 8 ";			//fire_2
	command<<"0";
	//command<<"\"";
	system(command.str().c_str());
	...
	command<<!gasState;						//reverse the reading so that 1 means danger
	//command<<"\"";
	system(command.str().c_str());
	...
	command<<!waterLeakageState;		//reverse the reading so that 1 means danger
	//command<<"\"";
	system(command.str().c_str());
				...
				if(strcmp("%",argv[2]) == 0){
					...
					if(argc>=4){
						if(strcmp("server",argv[3])==0){
							//sendToServer();
							send_to_server = 1; //user requested to update database
						}
					}
				}
